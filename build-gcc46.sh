...
VERSION_GCC=4.6.3
PREFIX=$INSTALL/gcc-${VERSION_GCC}-${DIST_VERSION}-${ARCH}-${PLATFORM}

# Extract GCC and its dependencies.
cd $SRC
rm -fr gcc-${VERSION_GCC}
tar xfj $TOP/gcc-core-${VERSION_GCC}.tar.bz2
tar xfj $TOP/gcc-g++-${VERSION_GCC}.tar.bz2
cd $SRC/gcc-${VERSION_GCC}
tar xfj $TOP/gmp-5.0.2.tar.bz2
mv gmp-5.0.2 gmp
